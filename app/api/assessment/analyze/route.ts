...
// Define the response schema for both languages
...
const BilingualAssessmentAnalysisSchema = z.object({
  th: AssessmentAnalysisSchema,
  en: AssessmentAnalysisSchema,
})
...
คุณต้องตอบเป็นภาษาไทยเท่านั้น`,
      ...
      basic: `คุณเป็นแพทย์เวชศาสตร์ป้องกันที่มีประสบการณ์ 15 ปี คุณจะประเมินสุขภาพเบื้องต้นของผู้ป่วย
...
You must respond ONLY in English.`,
      ...
      basic: `You are a preventive medicine physician with 15 years of experience. You will assess basic health status of patients.
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("🔍 API: Received bilingual analysis request")
    ...
    // Format answers for AI analysis (language-neutral)
...
Score: ${answer.score}`
    ...
    // Generate analysis for both languages
    console.log("🤖 API: Starting bilingual AI analysis...")

    // Thai analysis
    const thaiPrompt = `กรุณาวิเคราะห์ผลการประเมิน "${categoryTitle}" จากข้อมูลต่อไปนี้:
...
สำคัญ: คำตอบทั้งหมดต้องเป็นภาษาไทยเท่านั้น`
    ...
    // English analysis
    const englishPrompt = `Please analyze the "${categoryTitle}" assessment results from the following data:
...
IMPORTANT: All responses must be in English only.`
    ...
    console.log("🇹🇭 API: Generating Thai analysis...")
    const { object: thaiAnalysis } = await generateObject({
      model: openai("gpt-4o"),
      system: getSystemPrompt(categoryId, "th"),
      prompt: thaiPrompt,
      schema: AssessmentAnalysisSchema,
    })

    // Generate English analysis
    console.log("🇺🇸 API: Generating English analysis...")
    const { object: englishAnalysis } = await generateObject({
      model: openai("gpt-4o"),
      system: getSystemPrompt(categoryId, "en"),
    ...
    console.log("✅ API: Bilingual analysis completed successfully")
    console.log("🔍 API: Thai analysis sample:", {
      riskLevel: thaiAnalysis.riskLevel,
      riskFactorsCount: thaiAnalysis.riskFactors.length,
      recommendationsCount: thaiAnalysis.recommendations.length,
    })
    console.log("🔍 API: English analysis sample:", {
      riskLevel: englishAnalysis.riskLevel,
      riskFactorsCount: englishAnalysis.riskFactors.length,
      recommendationsCount: englishAnalysis.recommendations.length,
    })
